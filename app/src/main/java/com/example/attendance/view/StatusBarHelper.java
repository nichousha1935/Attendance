...
public class StatusBarHelper {
    ...
    public static void translucent(Dialog dialog) {
        translucent(dialog, 0x40000000);
    }
...
//            }
     ...
     * 支持 4.4 以上版本的 MIUI 和 Flyme，以及 5.0 以上版本的其他 Android。
     *
     * @param dialog 需要被设置沉浸式状态栏的 dialog。
     */
    @TargetApi(19)
    public static void translucent(Dialog dialog, @ColorInt int colorOn5x) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            // 版本小于4.4，绝对不考虑沉浸式
            return;
        }
...
//            }
     ...
     * 支持 4.4 以上版本 MIUI 和 Flyme，以及 6.0 以上版本的其他 Android
     *
     * @param dialog 需要被处理的 dialog
     */
    public static boolean setStatusBarLightMode(Dialog dialog) {
        if (dialog == null){
            return false;
        }
        // 无语系列：ZTK C2016只能时间和电池图标变色。。。。
        ...
        if (mStatuBarType != STATUSBAR_TYPE_DEFAULT) {
            return setStatusBarLightMode(dialog, mStatuBarType);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isMIUICustomStatusBarLightModeImpl() && MIUISetStatusBarLightMode(dialog.getWindow(), true)) {
                mStatuBarType = STATUSBAR_TYPE_MIUI;
                return true;
                ...
                Android6SetStatusBarLightMode(dialog.getWindow(), true);
                mStatuBarType = STATUSBAR_TYPE_ANDROID6;
                return true;
            }
        }
     ...
     * 支持 4.4 以上版本 MIUI 和 Flyme，以及 6.0 以上版本的其他 Android
     *
     * @param dialog 需要被处理的 Activity
     * @param type   StatusBar 类型，对应不同的系统
     */
    private static boolean setStatusBarLightMode(Dialog dialog, @StatusBarType int type) {
        if (type == STATUSBAR_TYPE_MIUI) {
            return MIUISetStatusBarLightMode(dialog.getWindow(), true);
        } else if (type == STATUSBAR_TYPE_FLYME) {
            return FlymeSetStatusBarLightMode(dialog.getWindow(), true);
        } else if (type == STATUSBAR_TYPE_ANDROID6) {
            return Android6SetStatusBarLightMode(dialog.getWindow(), true);
        }
        return false;
    }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (DeviceHelper.isTablet(context)
                && sStatusbarHeight > DeviceUtils.dip2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
            //状态栏高度大于25dp的平板，状态栏通常在下方
            sStatusbarHeight = 0;
        } else {
            if (sStatusbarHeight <= 0
                    || sStatusbarHeight > DeviceUtils.dip2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP * 2)) {
